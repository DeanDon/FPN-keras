...
@software: AI
@file: model_fpn.py.py
@time: 2017/11/14 14:32
@desc:
'''
"""
FPN
The main FPN model implemenetation.
...
class DetectionTargetLayer(KE.Layer):
    ...
    def call(self, inputs):
        ...
        outputs = utils.batch_slice(
            [proposals, gt_boxes],
            lambda x, y: detection_targets_graph(x, y, self.config),
...
def refine_detections(rois, probs, deltas, window, config):
    ...
    keep = np.where(probs[:, 1:] > config.DETECTION_MIN_CONFIDENCE)[0]
    #print "DUBUG::keep", keep.shape, keep
    # Filter out low confidence boxes
    # if config.DETECTION_MIN_CONFIDENCE:
    #     #keep1=np.where(class_scores >= config.DETECTION_MIN_CONFIDENCE)[0]
    #     keep = np.intersect1d(
    #          keep, np.where(class_scores >= config.DETECTION_MIN_CONFIDENCE)[0])
    # Apply per-class NMS
    #print "DEBUG::keep",keep.shape
    #print "DEBUG::keep1",keep1.shape
...
def refine_detections_myway(rois, probs, deltas, window, config):
    """Refine classified proposals and filter overlaps and return final
    detections.
...
def load_image_gt(dataset, config, image_id, augment=False,
    ...
    #print "DEBUG::bbox format",bbox,bbox.dtype
    #print "DEBUG::bbox class",class_ids
    ...
    #cv2.rectangle(image, (bbox[0][1], bbox[0][0]), (bbox[0][3], bbox[0][2]), color=(0, 255, 0), thickness=1)
    #cv2.imwrite('./test_bbox/' + str(image_id) + '.jpg', image)
    ...
    #print "DEBUG::bbox",image_id,bbox
    return image, image_meta, bbox


def load_image_gt_without_mask(dataset, config, image_id, augment=False,
                  use_mini_mask=False):
    """Load and return ground truth data for an image (image, mask, bounding boxes).
    ...
    """
    # Load image and mask
    #print "dataset image_info path",dataset.image_info[image_id]['path']
    image = dataset.load_image(image_id)
    image_copy=image.copy()
    gt, class_ids = dataset.load_gt(image_id)
    #cv2.rectangle(image_copy, (gt[0][0], gt[0][1]), (gt[0][2], gt[0][3]), color=(0, 0, 255), thickness=2)
    #cv2.imwrite('./test_bbox/ori_' + str(image_id) + '.jpg', image_copy)
    shape = image.shape
    image, window, scale, padding = utils.resize_image(
        image,
        min_dim=config.IMAGE_MIN_DIM,
        max_dim=config.IMAGE_MAX_DIM,
        padding=config.IMAGE_PADDING)
    gt = utils.resize_gt(gt, scale, padding)
    ...
    #print "DEBUG::bbox class",class_ids
    # Add class_id as the last value in bbox
    bbox = np.hstack([gt, class_ids[:, np.newaxis]])
    ...
    active_class_ids = np.zeros([dataset.num_classes], dtype=np.int32)
    class_ids = dataset.source_class_ids[dataset.image_info[image_id]["source"]]
    #print "DEBUG::class_ids",class_ids
    active_class_ids[class_ids] = 1

    # Image meta data
    image_meta = compose_image_meta(image_id, shape, window, active_class_ids)
    #print "DEBUG::bbox",image_id,bbox
    #cv2.rectangle(image,(bbox[0][1],bbox[0][0]),(bbox[0][3],bbox[0][2]),color=(0,0,255),thickness=2)
    #cv2.imwrite('./test_bbox/'+str(image_id)+'.jpg',image)
    return image, image_meta, bbox


def build_detection_targets(rpn_rois, gt_boxes, config):
    ...
    while True:
        try:
            ...
            if shuffle and image_index == 0:
                ...
                #open('./trainset.txt','a').write('\n'.join([dataset.image_info[image_id]['path'] for image_id in image_ids]))
                #open('./trainset.txt', 'a').write('---------------------------------------------\n')
            ...
            if random_rois:
                ...
                if detection_targets:
                    ...
                    rois, mrcnn_class_ids, mrcnn_bbox = \
                        build_detection_targets(rpn_rois, gt_boxes, config)
            ...
            if b >= batch_size:
                ...
                #print "DEBUG::image_ids[0:10]",image_ids[0:10]
...
class FPN():
    ...
    def build(self, mode, config):
        ...
        if mode == "training":
            ...
            inputs = [input_image, input_image_meta,
                      input_rpn_match, input_rpn_bbox, input_gt_boxes]
            if not config.USE_RPN_ROIS:
    ...
    def mold_inputs(self, images):
        ...
        for image in images:
            ...
            #cv2.imwrite('1024_1.jpg', molded_image)
            # Build image_meta
    ...
    def detect(self, images, verbose=0):
        ...
        #cv2.imwrite('512_.jpg', molded_images[0])
        #if verbose:
